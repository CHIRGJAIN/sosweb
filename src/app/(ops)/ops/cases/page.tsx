    ...
    <div className="space-y-4">
      ...
      <FilterBar>
        <input
          ...
          className="w-64 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-amber-500 focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-white"
        />
      </FilterBar>
      <DataTable<CaseRecord> data={filtered} columns={columns} onRowClick={(row) => router.push(`/ops/cases/${row.id}`)} />
    </div>
  );
}
