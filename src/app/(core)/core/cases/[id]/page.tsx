...
export default function CoreCaseDetailPage() {
  ...
  const assignMutation = useMutation({
    mutationFn: (assignee: string) =>
      mockApi.assignCase({ caseId: params.id, assignee, actor: session?.name ?? "Core Admin", role: session?.role ?? "CORE_ADMIN" }),
    onSuccess: (data) => {
  ...
  if (caseQuery.isLoading || !caseQuery.data) {
    return <div className="p-6 text-sm text-slate-600">Loading case...</div>;
  }
